  ...
  try {
    ...
    vehicleIdentifiers.forEach((vehicleInfo, index) => {
      try {
        ...
        const vehicleUuidPattern = new RegExp(`https://prod\\.pictures\\.autoscout24\\.net/listing-images/${escapedVehicleId}_[a-f0-9-]+\\.(?:jpg|jpeg|webp)([^"'\s]*)`, 'i');
        ...
        if (image && image.includes('250x188') && vehicleHtml.includes('480x360')) {
        ...
        if (yearDetailMatch && yearDetailMatch[2]) {
          ...
          const yearMatch = vehicleHtml.match(/data-testid="[^"]*year[^"]*"[^>]*>[\s\S]*?(\d{4})/i) ||
                           ...
                           vehicleHtml.match(/\b(19|20)\d{2}\b/);
          if (yearMatch) {
            const yearStr = yearMatch[2] || yearMatch[1] || yearMatch[0];
