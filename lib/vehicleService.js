...
function parseAutoscoutFromHtml(html, category, dealerSlug) {
  ...
  try {
    ...
    // SECOND: Extract vehicle identifiers from links or data attributes
    // AutoScout24 uses various patterns: /fahrzeugdetails?vid=, data-vehicle-id, etc.
    if (vehicleIdentifiers.length === 0) {
      // Try to find vehicle IDs in links
      ...
      const allIds = [...new Set([...idsFromLinks, ...idsFromData])];
      
      vehicleIdentifiers = allIds.map(id => ({ id, url: null, structuredData: null }));
      console.log(`Found ${vehicleIdentifiers.length} vehicle identifiers from links/data attributes`);
    }
    ...
    if (vehicleIdentifiers.length === 0) {
      console.warn("No vehicle identifiers found in AutoScout24 HTML");
      return [];
    }
    ...
    vehicleIdentifiers.forEach((vehicleInfo, index) => {
      try {
        ...
        // Method 1: Look for article or listing element containing the ID
        const articlePattern = new RegExp(`<article[^>]*[^>]*>([\\s\\S]*?${vehicleId}[\\s\\S]*?)<\\/article>`, 'i');
        ...
        if (!vehicleHtml) {
          const dataPattern = new RegExp(`<div[^>]*data-vehicle-id=["']${vehicleId}["'][^>]*>([\\s\\S]*?)(?=<div[^>]*data-vehicle-id|$)`, 'i');
        ...
        if (!vehicleHtml && vehicleId.match(/^[a-f0-9-]+$/)) {
          const uuidPattern = new RegExp(`([\\s\\S]{0,3000}?listing-images\\/${vehicleId}_[\\s\\S]{0,3000}?)`, 'i');
    ...
    // SECOND: Extract ALL listing images with their positions
    // AutoScout24 image URL pattern: https://prod.pictures.autoscout24.net/listing-images/{UUID}_{UUID}.jpg/{size}.webp
    const allImagesRegex = /https:\/\/prod\.pictures\.autoscout24\.net\/listing-images\/([a-f0-9-]+)_([a-f0-9-]+)\.(?:jpg|jpeg|webp)([^"'\s]*)/gi;
    const imageMatches = [];
    let match;
    while ((match = allImagesRegex.exec(html)) !== null) {
      imageMatches.push({
        fullUrl: match[0],
        vehicleUuid: match[1],
        imageUuid: match[2],
    ...
    console.log(`Found ${imagesByVehicleUuid.size} unique vehicle images from AutoScout24`);
    
    // THIRD: Find vehicle listings by looking for article tags or specific patterns
    // AutoScout24 uses article tags or divs with specific classes
    const listingPattern = /<article[^>]*>[\s\S]{0,5000}?<\/article>/gi;
    const listings = [...html.matchAll(listingPattern)];
    
    // If no article tags, try to find vehicle blocks by price patterns
    if (listings.length === 0) {
      // Split by price patterns but keep context
      ...
      priceMatches.forEach((priceMatch, idx) => {
        ...
        // Get context around this price (2000 chars before, 1000 after)
        const startPos = Math.max(0, priceMatch.index - 2000);
        const endPos = Math.min(html.length, priceMatch.index + 1000);
        const context = html.substring(startPos, endPos);
        
        // Find title - look for brand + model pattern or data attributes
        let title = null;
        
        // Try data-title or aria-label
        const dataTitleMatch = context.match(/data-title=["']([^"']{10,100})["']/i) || 
                               context.match(/aria-label=["']([^"']{10,100})["']/i);
        if (dataTitleMatch) {
          title = dataTitleMatch[1].trim();
        }
        
        // Try to find brand + model in text
        if (!title) {
        ...
        // If still no title, try to extract from slug in URL or data attributes
        if (!title) {
          const slugMatch = context.match(/([a-z0-9-]{20,80})/i);
          if (slugMatch) {
            // Convert slug to readable title
            title = slugMatch[1]
              .replace(/-/g, ' ')
              .replace(/\b\w/g, l => l.toUpperCase())
              .replace(/\b(Vw|Tdi|Bmt|Dsg|Kw|Ps)\b/gi, m => m.toUpperCase());
          }
        }
        ...
        const year = yearMatch ? parseInt(yearMatch[1]) : null;
        if (year && (year < 1970 || year > new Date().getFullYear() + 1)) return;
        
        const kmMatch = context.match(/([\d\s.]+)\s*km/i);
        const mileage = kmMatch ? parseInt(kmMatch[1].replace(/\s/g, '').replace(/\./g, '')) : null;
        
        const powerMatch = context.match(/(\d+)\s*kW\s*\((\d+)\s*PS\)/);
        const power = powerMatch ? { kw: parseInt(powerMatch[1]), ps: parseInt(powerMatch[2]) } : null;
        
        let fuelType = null;
        if (context.includes('Diesel')) fuelType = 'Diesel';
        else if (context.includes('Benzin')) fuelType = 'Benzin';
        else if (context.includes('Elektro')) fuelType = 'Elektro';
        
        let transmission = null;
        if (context.includes('Automatik')) transmission = 'Automatik';
        else if (context.includes('Schaltgetriebe')) transmission = 'Schaltgetriebe';
        
        // Find image - look for image URL in this specific context
        let image = null;
        ...
        if (imageInContext) {
          ...
          if (imagesByVehicleUuid.has(vehicleUuid)) {
            image = imagesByVehicleUuid.get(vehicleUuid).fullUrl;
          } else {
            image = imageInContext[0];
          }
        }
      ...
      listings.forEach((listingMatch, idx) => {
        const listingHtml = listingMatch[0];
        
        // Extract title from article
        let title = null;
        const titleMatch = listingHtml.match(/<h[23][^>]*>([^<]{10,100})<\/h[23]>/i) ||
                          listingHtml.match(/data-title=["']([^"']{10,100})["']/i) ||
                          listingHtml.match(/aria-label=["']([^"']{10,100})["']/i);
        if (titleMatch) {
        ...
        // Extract other details
        const yearMatch = listingHtml.match(/(\d{4})/);
        const year = yearMatch ? parseInt(yearMatch[1]) : null;
        ...
        // Find image in article
        let image = null;
        const imageMatch = listingHtml.match(/https:\/\/prod\.pictures\.autoscout24\.net\/listing-images\/([a-f0-9-]+)_[a-f0-9-]+\.(?:jpg|jpeg|webp)[^"'\s]*/i);
        if (imageMatch) {
          const vehicleUuid = imageMatch[0].match(/\/([a-f0-9-]+)_/)[1];
          if (imagesByVehicleUuid.has(vehicleUuid)) {
            image = imagesByVehicleUuid.get(vehicleUuid).fullUrl;
          } else {
            image = imageMatch[0];
    ...
    const uniqueVehicles = vehicles.filter(v => {
      const key = `${v.title}-${v.price}`;
      if (seen.has(key)) return false;
      seen.add(key);
      return true;
    });
    
    console.log(`Found ${uniqueVehicles.length} unique vehicles from AutoScout24 HTML`);
  ...
  try {
    ...
    if (nuxtDataMatch) {
      try {
        ...
        // The Nuxt data array contains all page data - look for image URLs
        if (Array.isArray(jsonData)) {
          // Find image URLs in the JSON array
          const jsonImages = jsonData.filter(item => 
            typeof item === 'string' && 
            item.includes('images.landwirt.com') &&
            (item.endsWith('.jpg') || item.endsWith('.jpeg') || item.endsWith('.webp'))
          );
          
          console.log(`Found ${jsonImages.length} images in NUXT_DATA`);
          
          // Add these to our image pool
          if (jsonImages.length > 0) {
            uniqueImages.push(...jsonImages);
          }
        }
      } catch (e) {
    ...
    // Assign images to machines
    knownMachines.forEach((machine, idx) => {
      // Try to find a specific image for this machine
      let image = null;
      
      // Method 1: Look for machine-related keywords in image URLs
      const machineKeywords = machine.title.toLowerCase().split(/[\s,]+/).filter(k => k.length > 3);
      for (const keyword of machineKeywords) {
        const matchingImage = uniqueImages.find(url => url.toLowerCase().includes(keyword));
        if (matchingImage) {
          image = matchingImage;
          console.log(`Matched image by keyword '${keyword}' for ${machine.title}`);
          break;
        }
      }
      
      // Method 2: Sequential assignment from image pool
      if (!image && uniqueImages.length > idx) {
        image = uniqueImages[idx];
        console.log(`Assigned image ${idx} from pool to ${machine.title}`);
      }
      ...
      console.log(`Found machine: ${machine.title} - €${machine.price || 'Preis auf Anfrage'} ${image ? '(with image)' : '(no image)'}`);
    });
