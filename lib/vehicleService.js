...
function parseLandwirtMachines(html) {
  ...
  try {
    ...
    if (machineIdentifiers.length === 0) {
      // Try to find script tag containing __NUXT__
      const nuxtScriptMatch = html.match(/<script[^>]*>[\s\S]*?window\.__NUXT__\s*=\s*({[\s\S]{0,50000}?});[\s\S]*?<\/script>/i);
      if (nuxtScriptMatch && nuxtScriptMatch[1]) {
        try {
          const jsonData = JSON.parse(nuxtScriptMatch[1]);
          // Extract machines from Nuxt state
          if (jsonData && jsonData.state) {
            // Try various paths where machines might be stored
            const machines = jsonData.state.machines || 
                           jsonData.state.data?.machines ||
                           jsonData.state.data?.items ||
                           (jsonData.state.data && Array.isArray(jsonData.state.data) ? jsonData.state.data : []);
            
            if (Array.isArray(machines) && machines.length > 0) {
              machines.forEach((machine, idx) => {
                if (machine && (machine.id || machine.slug || machine.title || machine.name)) {
                  machineIdentifiers.push({
          ...
          console.log("window.__NUXT__ data not parseable, trying other methods");
        }
      }
    }
