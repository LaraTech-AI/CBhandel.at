...
function parseAutoscoutFromHtml(html, category, dealerSlug) {
  ...
  try {
    // FIRST: Try to extract from JSON-LD or script tags with vehicle data
    const jsonLdRegex = /<script[^>]*type=["']application\/ld\+json["'][^>]*>(.*?)<\/script>/gs;
      ...
      try {
        ...
        if (data['@type'] === 'ItemList' && Array.isArray(data.itemListElement)) {
          ...
          data.itemListElement.forEach((item, idx) => {
            if (item.item && item.item['@type'] === 'Product' || item.item['@type'] === 'Vehicle') {
              const vehicle = item.item;
              const title = vehicle.name || vehicle.headline || null;
              const price = vehicle.offers?.price || vehicle.price || null;
              
              if (title && price) {
                vehicles.push({
                  id: `autoscout-${vehicle.identifier || vehicle.sku || idx}`,
                  title: title,
                  ...
                  url: vehicle.url || `https://www.autoscout24.at/haendler/${dealerSlug}`,
                  category: category
                });
              }
            }
          ...
          if (vehicles.length > 0) {
            console.log(`Extracted ${vehicles.length} vehicles from JSON-LD`);
            return vehicles;
          }
        }
      } catch (e) {
        // Not valid JSON or not vehicle data, continue
      }
    }
