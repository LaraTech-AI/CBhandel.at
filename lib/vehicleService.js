...
function parseAutoscoutFromHtml(html, category, dealerSlug) {
  ...
  try {
    // FIRST: Extract ALL listing images from the entire HTML
    ...
    const imageMatches = [...html.matchAll(allImagesRegex)];
    
    // Create a map of vehicle UUIDs to their images
    const imagesByUuid = new Map();
    imageMatches.forEach(match => {
      const fullUrl = match[0];
      const uuid = match[1]; // First UUID is the vehicle identifier
      if (!imagesByUuid.has(uuid)) {
        imagesByUuid.set(uuid, fullUrl);
      }
    });
    
    console.log(`Found ${imagesByUuid.size} unique vehicle images from AutoScout24`);
    
    // Also collect ALL unique images as a fallback pool
    const allImages = [...new Set(imageMatches.map(m => m[0]))];
    console.log(`Total listing images found: ${allImages.length}`);
    ...
    let imageIndex = 0; // Track which image to assign
    ...
    vehicleChunks.forEach((chunk, index) => {
      ...
      // Get a larger context around the title for image and UUID search
      const extendedSearchStart = titleStartIndex >= 0 ? Math.max(0, titleStartIndex - 5000) : 0;
      const extendedSearchEnd = titleStartIndex >= 0 ? Math.min(html.length, titleStartIndex + 5000) : combinedContent.length;
      const extendedSearchArea = titleStartIndex >= 0 
        ? html.substring(extendedSearchStart, extendedSearchEnd)
        : combinedContent;
      ...
      // Method 1: Look for UUID in the extended search area and match to image
      const uuidInAreaMatch = extendedSearchArea.match(/listing-images\/([a-f0-9-]{36})/i);
      if (uuidInAreaMatch && imagesByUuid.has(uuidInAreaMatch[1])) {
        image = imagesByUuid.get(uuidInAreaMatch[1]);
        console.log(`Matched image by UUID for ${title.substring(0, 30)}...`);
      }
      
      // Method 2: Direct image match in the area
      if (!image) {
        const directImgMatch = extendedSearchArea.match(/https:\/\/prod\.pictures\.autoscout24\.net\/listing-images\/[a-f0-9-]+_[a-f0-9-]+\.(?:jpg|jpeg|webp)[^"'\s]*/i);
        if (directImgMatch) {
          image = directImgMatch[0];
          console.log(`Direct image match for ${title.substring(0, 30)}...`);
        }
      }
      
      // Method 3: Use sequential assignment from image pool as fallback
      if (!image && allImages.length > imageIndex) {
        image = allImages[imageIndex];
        imageIndex++;
        console.log(`Assigned image ${imageIndex} from pool to ${title.substring(0, 30)}...`);
      }
      ...
      console.log(`Found vehicle: ${title} - €${price} ${image ? '(with image)' : '(no image)'}`);
    });
