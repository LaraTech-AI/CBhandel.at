  ...
  try {
    ...
    vehicleIdentifiers.forEach((vehicleInfo, index) => {
      try {
        ...
        // Extract price - multiple fallback patterns
        const priceMatch = vehicleHtml.match(/data-testid="[^"]*price[^"]*"[^>]*>€\s*([\d.]+)/) ||
                          vehicleHtml.match(/€\s*([\d\s.]+)/) ||
                          vehicleHtml.match(/price["']?\s*:\s*["']?([\d.]+)/i);
        ...
        // Extract image - look for AutoScout24 image pattern
        let image = null;
        ...
        // Extract mileage
        const kmMatch = vehicleHtml.match(/data-testid="[^"]*mileage[^"]*"[^>]*>[\s\S]*?([\d.]+)/i) ||
        ...
        // Get URL
        const urlMatch = vehicleHtml.match(/href=["']([^"']*fahrzeugdetails[^"']*)["']/i);
...
function parseLandwirtMachines(html) {
  ...
  try {
    ...
    if (machineIdentifiers.length === 0) {
      console.warn("No machine identifiers found in Landwirt.com HTML");
      return [];
    }
