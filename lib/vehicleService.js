  ...
  try {
    ...
    listings.forEach((listingHtml, index) => {
      try {
        ...
        // Extract image
        const imgMatch = listingHtml.match(/src="(https:\/\/[^"]*autoscout24[^"]*\.jpg[^"]*)"/i) ||
                        listingHtml.match(/src="(https:\/\/[^"]*\.jpg[^"]*)"/i);
        const image = imgMatch ? imgMatch[1] : null;
  ...
  try {
    ...
    vehicleChunks.forEach((chunk, index) => {
      ...
      const imgPatterns = [
        ...
        /background-image:\s*url\(['"]?([^'")]*\.(?:jpg|jpeg|png|webp)[^'")]*)['"]?\)/i
      ];
      
      for (const pattern of imgPatterns) {
        const imgMatch = combinedContent.match(pattern) || chunk.match(pattern);
        if (imgMatch && imgMatch[1]) {
          image = imgMatch[1].split(',')[0].trim(); // Take first URL from srcset if multiple
          if (image.startsWith('//')) image = 'https:' + image;
          if (image.startsWith('/')) image = 'https://www.autoscout24.at' + image;
          break;
        }
      }
