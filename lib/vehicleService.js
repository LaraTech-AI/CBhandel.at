...
function parseLandwirtMachines(html) {
  ...
  try {
    ...
    if (machineIdentifiers.length === 0) {
      // Try window.__NUXT__ format
      const windowNuxtMatch = html.match(/window\.__NUXT__\s*=\s*({[\s\S]*?});/);
      if (windowNuxtMatch) {
        try {
          const jsonData = JSON.parse(windowNuxtMatch[1]);
          // Extract machines from Nuxt state
          if (jsonData.state && jsonData.state.machines) {
            const nuxtMachines = Array.isArray(jsonData.state.machines) ? jsonData.state.machines : [];
            nuxtMachines.forEach((machine, idx) => {
              if (machine.id || machine.slug) {
                machineIdentifiers.push({
          ...
          console.error("Error parsing window.__NUXT__ data:", e.message);
        }
      }
    }
