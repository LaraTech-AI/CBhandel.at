  ...
  try {
    ...
    vehicleIdentifiers.forEach((vehicleInfo, index) => {
      try {
        ...
        const articleIdPattern = new RegExp(`<article[^>]*id=["']${vehicleId}["'][^>]*>([\\s\\S]*?)<\\/article>`, 'i');
        ...
        let title = null;
        const h2Match = vehicleHtml.match(/<h2[^>]*>([^<]{5,50})<\/h2>/i);
        const versionMatch = vehicleHtml.match(/<span[^>]*class=["'][^"']*version["'][^>]*>([^<]{5,80})<\/span>/i);
        if (h2Match && versionMatch) {
          title = `${h2Match[1].trim()} ${versionMatch[1].trim()}`.replace(/\s+/g, ' ').trim();
        } else if (h2Match) {
        ...
        // Extract image - look for AutoScout24 image pattern (match vehicle UUID)
        let image = null;
        // First try to match image with same vehicle UUID
        const vehicleUuidPattern = new RegExp(`https://prod\\.pictures\\.autoscout24\\.net/listing-images/${vehicleId}_[a-f0-9-]+\\.(?:jpg|jpeg|webp)([^"'\s]*)`, 'i');
        const uuidImgMatch = vehicleHtml.match(vehicleUuidPattern);
        if (uuidImgMatch) {
          image = uuidImgMatch[0];
          // Prefer higher quality images
          if (image.includes('/250x188')) {
            image = image.replace('/250x188', '/480x360');
          }
        } else {
          // Fallback: any AutoScout24 image in the section
          const imgMatch = vehicleHtml.match(/https:\/\/prod\.pictures\.autoscout24\.net\/listing-images\/([a-f0-9-]+)_[a-f0-9-]+\.(?:jpg|jpeg|webp)([^"'\s]*)/i) ||
                           vehicleHtml.match(/src=["'](https:\/\/prod\.pictures\.autoscout24\.net[^"']+)["']/i) ||
                           vehicleHtml.match(/data-src=["'](https:\/\/prod\.pictures\.autoscout24\.net[^"']+)["']/i);
          if (imgMatch) {
            image = imgMatch[0] || imgMatch[1];
            // Prefer higher quality images
          ...
          if (image.includes('250x188') && vehicleHtml.includes('480x360')) {
            const largerMatch = vehicleHtml.match(/https:\/\/prod\.pictures\.autoscout24\.net\/listing-images\/[^"'\s]*480x360[^"'\s]*/i);
            if (largerMatch) image = largerMatch[0];
          }
        }
        ...
        const yearMatch = vehicleHtml.match(/data-testid="[^"]*year[^"]*"[^>]*>[\s\S]*?(\d{4})/i) ||
                         ...
                         vehicleHtml.match(/(19|20)\d{2}/);
        const year = yearMatch ? parseInt(yearMatch[1]) : null;
        if (year && (year < 1970 || year > new Date().getFullYear() + 1)) {
          // Invalid year, set to null
        }
        ...
        const finalVehicleId = `autoscout-${slugifiedTitle}-${price}`;
...
function parseLandwirtMachines(html) {
  ...
  try {
    ...
    // Try Nuxt data (window.__NUXT__ format)
    if (machineIdentifiers.length === 0) {
      // Try window.__NUXT__ format - be more careful with regex to avoid matching too much
      const windowNuxtMatch = html.match(/window\.__NUXT__\s*=\s*({[^;]*?});/);
      if (windowNuxtMatch && windowNuxtMatch[1]) {
        try {
          // Limit the JSON size to prevent issues
          const jsonStr = windowNuxtMatch[1].substring(0, 100000); // Limit to 100KB
          const jsonData = JSON.parse(jsonStr);
          // Extract machines from Nuxt state
          if (jsonData && jsonData.state && jsonData.state.machines) {
            const nuxtMachines = Array.isArray(jsonData.state.machines) ? jsonData.state.machines : [];
            nuxtMachines.forEach((machine, idx) => {
              if (machine && (machine.id || machine.slug)) {
                machineIdentifiers.push({
                  id: machine.id || machine.slug || `nuxt-${idx}`,
                  url: machine.url || null,
                  structuredData: machine
                });
              }
            });
            if (machineIdentifiers.length > 0) {
              console.log(`Found ${machineIdentifiers.length} machines from window.__NUXT__`);
            }
          }
        } catch (e) {
          // Silently fail - window.__NUXT__ might not be in expected format
          console.log("window.__NUXT__ data not in expected format, trying other methods");
        }
      }
    }
    ...
    // THIRD: Extract from HTML links or use image URLs as fallback
    if (machineIdentifiers.length === 0) {
      const machineLinkRegex = /href=["'][^"']*\/machine[^"']*\/([^"'\s\/]+)/gi;
      const linkMatches = [...html.matchAll(machineLinkRegex)];
      const idsFromLinks = [...new Set(linkMatches.map(m => m[1]))];
      ...
      console.log(`Found ${machineIdentifiers.length} machine identifiers from HTML`);
    }
    ...
    machineIdentifiers.forEach((machineInfo, index) => {
      try {
        ...
        // Find HTML section
        const articlePattern = new RegExp(`<article[^>]*[^>]*>([\\s\\S]*?${machineId}[\\s\\S]*?)<\\/article>`, 'i');
        const articleMatch = html.match(articlePattern);
